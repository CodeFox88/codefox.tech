	...
	$page_title = 'Life Escape - Your Wish List';
...
//add product to wishlist
	 if (isset ($pid, $sp_type, $_GET['action']) && ($_GET['action'] == 'add') ) { // Add a new product to wish list:

		$r = mysqli_query($dbc, "CALL add_to_wish_list('$uid', '$sp_type', $pid, 1)");
		header('Location: /450/html/wishlist.php');
//

	} elseif (isset ($sp_type, $pid, $_GET['action']) && ($_GET['action'] == 'remove') ) { // Remove it from the wish list.
